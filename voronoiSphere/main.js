...
export function fragmentShader() {
	...
	varying vec2 vUv;
	varying vec3 vDeltaPos;

	void main() {
		vec3 col = abs(vec3(sin(time)*0.8, cos(time)*0.8, sin(time)*exp(time)*0.8) - vDeltaPos);
...
export function vertexShader() {
	...
	void main() {
		vDeltaPos = voronoi(position);
		vec3 tmp = position * shift + vDeltaPos;
		vUv = uv;
